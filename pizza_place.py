...
def vieworder():
    ...
    for i in order:
        ...
        if i == 6:
            ...
            l += 12.99
        print(f"{k}. {j}")
    print(f"\nThe total cost of your items is ${l}\n")
    ...
    while user_exit_a != True:
        ...
        if y == 2:
            ...
            while x != -1:
                if x != -1:
                    if x >= 1 and x <= 6:
                        x = int(input("Enter which Pizzas you would\
                             like to order. Type -1 to finish: "))
                        order.append(x)
                    else:
                        print("Error: Invalid Item. Please enter a\
                             number 1-6")
                        order.pop()
                        orders()
            order.pop()
        ...
        if y == 3:
            ...
            while x != -1:
                item = "error"
                x = int(input("\nEnter the place of the item you want\
                     to remove, from the order list. Type -1 when you\
                     are finished: "))
                print(x)
                if x != -1:
                    x -= 1
